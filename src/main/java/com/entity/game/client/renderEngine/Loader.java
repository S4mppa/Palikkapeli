...
import com.entity.game.client.textures.TextureData;
...
public class Loader {
    ...
    public Mesh loadToVAO(float[] vertices, int[] indices, float[] texCoords, float[] normals, float[] blockTypes){
        int vaoID = createVAO();
        bindIndexBuffer(indices);
        storeDataInAttributeList(0, vertices, 3);
        storeDataInAttributeList(1, texCoords, 2);
        storeDataInAttributeList(2, normals, 3);
        storeDataInAttributeList(3, blockTypes, 1);
    ...
    {
        try {
            return ImageIO.read(Loader.class.getResourceAsStream("/"+loc));
        } catch (IOException e) {
            //Error Handling Here
            e.printStackTrace();
        }
    ...
    public int loadCubeMap(String[] textureFiles){
        ...
        for(int i = 0; i< textureFiles.length; i++){
            TextureData data = loadToBuffer("skybox/" + textureFiles[i] + ".png");
